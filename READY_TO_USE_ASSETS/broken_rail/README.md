# broken_rail

Splintered gap marker with bent rails and warning lamp.

## Runtime file

Use `broken_rail.glb`. Load it once with Three.js `GLTFLoader`, then clone or
instance it when repeated.

## Placement

- Origin: lane centre at gap leading edge.
- Three.js up: `+Y`.
- Three.js gameplay forward: `+Z`.
- Gameplay collision remains in the existing track-space collision system.

## Animation library

The GLB timeline is authored at 30 fps. Use the frame ranges below to create Three.js
subclips from the embedded action library.

- Static asset; no baked animation clip.

## Runtime notes

- The actual gap collision is track-space geometry; this mesh marks its leading edge.
